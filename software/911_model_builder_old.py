...
def generate_smt2_file(df, output_file, num_samples=0, num_layers=2, nodes_per_layer=[5], use_relu=True, int_processing=False):
    """
    ...
    if num_layers == 0:
        ...
        for i, feature in enumerate(feature_names, start=1):
            if int_processing:
                smt_lines.append(f"(declare-fun x{i} () Int) ; Feature '{feature}'")
                smt_lines.append(f"(assert (and (>= x{i} -100) (<= x{i} 100)))")
            else:
                smt_lines.append(f"(declare-fun x{i} () Real) ; Feature '{feature}'")
                smt_lines.append(f"(assert (and (>= x{i} -100.0) (<= x{i} 100.0)))")
        if int_processing:
...
if __name__ == "__main__":
    ...
    generate_smt2_file(df, args.output_file, args.num_samples, args.num_layers, args.nodes_per_layer, use_relu=not args.norelu, int_processing=args.int)